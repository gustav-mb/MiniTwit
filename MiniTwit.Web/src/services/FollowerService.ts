...
class FollowerService {
    public async followUser(username: string | undefined, userId: string): Promise<undefined> {
        return await axiosClient.post(`${ROUTE}/${username}/follow?userId=${userId}`)
    }

    public async unfollowUser(username: string | undefined, userId: string): Promise<undefined> {
